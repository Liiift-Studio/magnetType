	...
	return new ImageResponse(
		(
			<div style={{ background: '#0a1520', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '72px 80px', fontFamily: 'Inter, sans-serif' }}>
				<span style={{ fontSize: 13, letterSpacing: '0.18em', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase' }}>magnet type</span>
				<div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
					<div style={{ display: 'flex', gap: 8, marginBottom: 48, alignItems: 'flex-end' }}>
						{[300, 380, 460, 560, 680, 780, 900].map((w, i) => (
							<div key={i} style={{ width: 32, height: Math.round(8 + (w - 300) / 600 * 40), background: `rgba(168,216,234,${0.12 + i * 0.1})`, borderRadius: 3 }} />
						))}
					</div>
					<div style={{ fontSize: 76, color: '#ffffff', lineHeight: 1.06, fontWeight: 300 }}>Magnet Type,</div>
					<div style={{ fontSize: 76, color: 'rgba(255,255,255,0.4)', lineHeight: 1.06, fontWeight: 300 }}>word by word.</div>
				</div>
				<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
					...
					<div style={{ fontSize: 13, color: 'rgba(255,255,255,0.18)', letterSpacing: '0.04em' }}>magnettype.com</div>
				</div>
			</div>
		),
